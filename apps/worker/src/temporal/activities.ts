/**
 * Temporal Activity Definitions (P0-M Track M1)
 *
 ...
 *
 * Each activity:
 *  1. Creates a fresh WorkflowContext (Temporal handles state, not us)
 *  2. Calls the corresponding private activity via a single-activity workflow run
 *  3. Returns a serializable result
 *
 * Temporal retry / timeout config is set in worker.ts, not here.
 */
...
async function runSingleActivity(
  ...
  // Map activity names to workflow methods
  const activityMap: Record<string, (wf: EpisodeProductionWorkflow) => Promise<WorkflowActivityResult>> = {
    "01_LOAD_EPISODE":       (w) => (w as any).activity01LoadEpisode(),
    "02_BUILD_EXECUTION_PLAN": (w) => (w as any).activity02BuildExecutionPlan(),
    "03_GENERATE_ASSETS":    (w) => (w as any).activity03GenerateAssets(),
    "04_VALIDATE_ASSETS":    (w) => (w as any).activity04ValidateAssets(),
    "05_REGISTER_ASSETS":    (w) => (w as any).activity05RegisterAssets(),
    "06_ASSEMBLE_SHOTS":     (w) => (w as any).activity06AssembleShots(),
    "07_ASSEMBLE_SCENES":    (w) => (w as any).activity07AssembleScenes(),
    "08_ASSEMBLE_EPISODE":   (w) => (w as any).activity08AssembleEpisode(),
    "09_GENERATE_AUDIO":     (w) => (w as any).activity09GenerateAudio(),
    "10_GENERATE_CAPTIONS":  (w) => (w as any).activity10GenerateCaptions(),
    "11_MENTOR_QA_REVIEW":   (w) => (w as any).activity11MentorQAReview(),
    "12_CONTINUITY_CHECK":   (w) => (w as any).activity12ContinuityCheck(),
    "13_HUMANIZATION_PASS":  (w) => (w as any).activity13HumanizationPass(),
    "14_REPAIR_LOOP":        (w) => (w as any).activity14RepairLoop(),
    "15_FINAL_RENDER":       (w) => (w as any).activity15FinalRender(),
    "16_GENERATE_THUMBNAILS":(w) => (w as any).activity16GenerateThumbnails(),
    "17_FINAL_QA":           (w) => (w as any).activity17FinalQA(),
    "18_RECORD_COMPLETION":  (w) => (w as any).activity18RecordCompletion(),
  };
...
// Each function is a plain async function — Temporal registers these via Worker.create({ activities })
